...
======*/
resource "aws_cloudwatch_log_group" "rails_postgres" {
  name = "rails_postgres"

  tags = {
    Environment = var.environment
    Application = "rails_postgres"
  }
}
...
======*/
resource "aws_ecr_repository" "rails_postgres_app" {
...
resource "aws_ecs_task_definition" "web" {
  ...
  container_definitions = templatefile("${path.module}/tasks/web_task_definition.json", {
    image           = aws_ecr_repository.rails_postgres_app.repository_url,
    ...
    log_group       = aws_cloudwatch_log_group.rails_postgres.name
  })
...
/*====
App Load Balancer
Before creating the Services, we need to create the load balancers.
They will be on the public subnet and will forward the requests to the ECS service.
======*/
...
resource "aws_alb" "alb_rails_postgres" {
  name            = "${var.environment}-alb-rails-postgres"
  subnets         = var.public_subnet_ids
  security_groups = [aws_security_group.web_inbound_sg.id]

  tags = {
    Name        = "${var.environment}-alb-rails_postgres"
    Environment = var.environment
  }
}


resource "aws_alb_listener" "rails_postgres" {
  load_balancer_arn = aws_alb.alb_rails_postgres.arn
  port              = "80"
  protocol          = "HTTP"
...
resource "aws_cloudwatch_metric_alarm" "service_cpu_high" {
  alarm_name          = "${var.environment}_rails_postgres_web_cpu_utilization_high"
